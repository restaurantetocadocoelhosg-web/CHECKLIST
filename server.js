...
app.put('/api/expiry/:id',(req,res)=>{const{name,category,expiry_date,quantity,update_date_only}=req.body;if(update_date_only){db.prepare('UPDATE expiry_items SET expiry_date=?,replaced=0,replaced_by=NULL,replaced_at=NULL,new_expiry_date=NULL WHERE id=?').run(expiry_date,req.params.id);}else{db.prepare('UPDATE expiry_items SET name=?,category=?,expiry_date=?,quantity=? WHERE id=?').run(name,category,expiry_date,quantity||null,req.params.id);}res.json({ok:true});});
